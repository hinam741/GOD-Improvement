...
class BaseLearner(object):
    ...
    def _compute_accuracy(self, model, loader):
        ...
        for i, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                outputs = model(inputs,self.lora_ids)["logits"]
            ...
            for _, _inputs, _targets in loader:
                ...
                if isinstance(self._network, nn.DataParallel):
                    ...
                    _vectors = tensor2numpy(
                        self._network.module.extract_vector(_inputs.to(self._device),self.lora_ids)
                    )
                else:
                    _vectors = tensor2numpy(
                        self._network.extract_vector(_inputs.to(self._device),self.lora_ids)
                    )
